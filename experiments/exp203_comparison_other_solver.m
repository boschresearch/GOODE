...
%  problems, with SE kernel. Also plots for Paper
%  Author: John
%  Creation: 2019-01-20
% -------------------------------------------------------------------
...
for ii = start:nbvp
    ...
    for jj = 1:M
        ...
        try
            ...
            [sol, info] = gpbvp_nonlinear_2(odefun, bcfun, dodefun, dbcfun, solinit, 'ls_fac', ls_fac, 'npoints', npoints);
            %disp(info)
            try % exact solution
                esol = esolus(sol.x);
                temp = esol(1,:);
...
TikzFigure('03_comparison_loglike_vs_global_opt.tex')
...
TikzFigure('04_comparison_all_solver_relerr.tex')
...
TikzFigure('04_comparison_all_solver_points.tex')
